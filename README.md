...
- При первом запуске БД автоматически наполняется стартовым набором
  категорий, единиц измерения и магазинов через versioned Alembic seed snapshot
- Аналитика и графики: динамика цен/инфляции по продукту, категории или
  магазину — индекс Ласпейреса, Пааше или Фишера на выбор
...
- **Сид стартовых данных** (`alembic/versions/a4ff3de9f63b_...py`) —
  категории/единицы/магазины грузятся из снимка
  `alembic/data/a4ff3de9f63b/*.csv`. Миграция вставляет только отсутствующие
  natural keys, поэтому безопасно обновляет уже заполненную БД и не падает
  на совпадающей категории/единице/магазине. Snapshot привязан к revision:
  изменение будущих seed CSV не меняет задним числом старую миграцию.
...
- В собранном GUI (`InflationTracker.exe`) в файл пишутся только `INFO` и
  выше: значимые действия пользователя, миграции, аналитика, предупреждения
  и ошибки.
- При запуске из исходников и в CLI дополнительно включается консоль с
  уровнем `DEBUG`. Техническая трассировка остаётся в консоли и не
  засоряет production-лог.
- Ожидаемые ошибки валидации пишутся как `WARNING` без traceback;
  неожиданные ошибки — как `ERROR`/`CRITICAL` со стеком вызовов.
...
Сборка делается через PyInstaller (Windows). Миграции Alembic упаковываются
в сборку и автоматически применяются при запуске. Стартовые справочники
также поставляются как versioned snapshot внутри `alembic/data/<revision>/`.

`app/data/seed/*.csv` остаётся рабочим источником для подготовки следующего
набора дефолтов. При выпуске новой data-миграции содержимое нужно скопировать
в новый `alembic/data/<revision>/` и после публикации revision не менять.
Так fresh install через год получает те же данные, что и в день релиза.
...
- Реализация расчёта "шринкфляции" (nullable `package_size` на `Purchase`);
- Экспорт/импорт покупок в CSV — резервная копия и перенос данных
  между машинами без ручного копирования файла БД;
- Пользовательская "корзина" — свой набор продуктов/категорий для
  персонального индекса инфляции, отдельно от индекса по всем покупкам;
- Шаблоны частых покупок ("повторить прошлый чек") — развитие
  пакетного ввода для регулярных закупок;
- Локальный автоматический бэкап файла SQLite по расписанию
  (ротация N последних копий, без облака и сети);
- Тёмная тема GUI.