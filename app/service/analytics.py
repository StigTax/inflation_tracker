...
def _period_start(dts: pd.Series, group_by: GroupBy) -> pd.Series:
    """Возвращает дату начала периода (datetime64[ns])."""
...
def _compute_price_and_spend(
    ...
    Добавляет:
    - unit_price_used
    - spend (unit_price_used * quantity)
    """
    if df.empty:
        return df
...
def _laspeyres_index(
    ...
    *,
    ...
    """
    Laspeyres-style индекс с весами из базового периода.
    ВАЖНО: если в периоде нет данных по части товаров, мы считаем индекс
    по доступной части корзины и возвращаем coverage
    (доля весов, покрытая данными).
    """
    if df.empty:
        return {
            "points": [],
...
def basket_inflation_index(
    *,
    ...
    """
    "Моя инфляция": индекс по выбранным продуктам
    (или по всем, если product_ids=None).
    """
    group_by = _ensure_group_by(group_by)
    price_mode = _ensure_price_mode(price_mode)
    promo_mode = _ensure_promo_mode(promo_mode)
...
def category_inflation_index(
    *,
    ...
    """
    Индекс инфляции внутри категории.
    """
    group_by = _ensure_group_by(group_by)
    price_mode = _ensure_price_mode(price_mode)
    promo_mode = _ensure_promo_mode(promo_mode)
...
def store_inflation_index(
    *,
    ...
    """
    Индекс инфляции по магазину (внутри магазина, со своими базовыми весами).
...
def product_store_price_stats(
    *,
    ...
    """
    "Где дешевле этот продукт в среднем" — агрегат по магазинам.
    Возвращает список магазинов, отсортированный по средней цене
    (avg_unit_price).
    """
    price_mode = _ensure_price_mode(price_mode)
    promo_mode = _ensure_promo_mode(promo_mode)
...
def inflation_contributions(
    *,
    ...
    """
    Вклад в инфляцию относительно базового периода (первый период).
    - by="product": вклад по продуктам
    - by="category": вклад по категориям
      (сумма вкладов продуктов внутри категории)
    """
    group_by = _ensure_group_by(group_by)
    price_mode = _ensure_price_mode(price_mode)
    promo_mode = _ensure_promo_mode(promo_mode)
